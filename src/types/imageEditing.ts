...
export interface ImageRegion {
  id: string;
  type: "decoration" | "icon" | "chart" | "image";
  bbox: BoundingBox;
  cos_key?: string;
  img_path?: string;
  confidence: number;
 ...
 * 装饰元素区域列表
 */
export interface ImageRegionList {
  regions: ImageRegion[];
  count: number;
  source: "mineru" | "manual";
}
...
export interface HybridOCRResult {
  ...
  image_regions?: ImageRegion[]; // 装饰元素区域
  metadata: HybridOCRMetadata | MinerUMetadata;
}
