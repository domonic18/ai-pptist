...
export interface PPTImageElement extends PPTBaseElement {
  ...
  imageInfo?: {
    ...
    filename?: string;
    cosKey: string;
    imageId?: string; // 数据库图片ID（可选，用于URL刷新）
    uploadTime?: number; // 上传时间戳（可选，用于判断URL是否可能过期）
  };
}
...
// 前向声明，避免循环依赖
// 这些类型的详细定义在 template-matching/types.ts 中
export type LogicType = string;
